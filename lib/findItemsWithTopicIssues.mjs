...
function writeTopicMismatchesToFile(itemList, fileName) {
  ...
  if (itemList.length > 0) {
    ...
    itemList.forEach((item, index) => {
      ...
      const agency = item['relatedQuestionGroup']['AgencyId']

      const fileData = {
        "agency": agency, "questionName": item['question']['ItemName']?.['en-GB'].trim(),
...
function writeItemsWithoutTopicsToFile(itemList, fileName) {
  ...
  if (itemList.length > 0) {
    ...
    itemList.forEach((item, index) => {
      ...
      const itemDescription = !!item['Summary'] ? item['Summary']?.['en-GB'].trim() : item['Label']?.['en-GB'].trim()
...
const count=0
for (const question of allQuestions['Results'].slice(3500,3700)) {
...
for (const variable of allVariables['Results'].slice(5000, 5300)) {
