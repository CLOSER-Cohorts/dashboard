...
export async function getToken(username, password) {
  ...
  export async function executePostRequestWithToken(url, token, requestBody) {
    ...
    try {
      ...
      throw new Error(`Error: 4 API call to ${url} failed! ${error.message}, ${error.cause}, ${error.cause.code}`);
    }
    ...
    try {
      ...
      else throw new Error(`Error: 5 API call to ${url} failed!`);
  ...
  export async function executeGetRequest(url, token) {
    ...
    try {
      ...
      console.log(`Error: 1 API call to ${url} failed! ${error.message}, ${error.cause}, ${error.cause.code}`)
    }
  ...
  export async function executeGetRequestWithoutToken(url) {
    ...
    try {
      ...
      console.log(`Error: 2 API call to ${url} failed! ${error.message}, ${error.cause}, ${error.cause.code}`)
    }
  ...
  export async function executePostRequestWithoutToken(url, username, password) {
    ...
    try {
      ...
      console.log(`Error: 3 API call to ${url} failed! ${error.message}, ${error.cause}, ${error.cause.code}`)
    }
