...
const currentNumberOfItemsInRepository = content['TotalItems']  
...
/*  if (err) {
  ...
  const previousNumberOfItemsInRepository = !err ? JSON.parse(data)['TotalItems'] : "NA"
...
// UPDATE FILE WITH NEW RECORDS AMOUNT
  ...
  if (err || (currentNumberOfItemsInRepository != previousNumberOfItemsInRepository)) {
    fs.writeFile('./lib/repositoryItemCount.txt', JSON.stringify(content), {flag: 'w'}, err => {
    if (err) {
      console.error(err);
    } else {
