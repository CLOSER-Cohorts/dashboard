export async function getToken(username, password) {
  ...
  return retVal
    ...
    console.log(`Error: authentication token generation request to ${url} failed. ${error}`)
  }
}
...
export async function getStudyGroups(token) {
  ...
  throw new Error(`Error: Collectica API call to ${url} failed!`); 
}
...
export async function executeGetRequest(url, token) {
  ...
  if (res.status === 200) return await res.json()
    ...
    console.log(`Error: Collectica API call to ${url} failed! ${error}`)
  }  
